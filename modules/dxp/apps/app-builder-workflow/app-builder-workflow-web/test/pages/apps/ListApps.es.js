...
const EDIT_PATH = `/workflow/:dataDefinitionId(\\d+)?/deploy/:appId(\\d+)?`;
...
describe('ListApps', () => {
	...
	it('renders with 5 apps on list and opens a new app tooltip', async () => {
		...
		expect(
			...
			'workflow-powered-appcreate-an-app-driven-by-workflow-process'
		);
	...
	it('renders with empty state', async () => {
		...
		expect(
			...
			'no-workflow-powered-apps-yetworkflow-powered-are-apps-driven-by-workflow-process'
		);
